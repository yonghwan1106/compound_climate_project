"""
Compact Research Report PDF Generator (9 Pages)
...
def create_compact_research_report_pdf(output_path: str, figures_dir: str):
    ...
    achieves F1-score of 0.89 for event detection and R² of 0.82 for impact prediction. Under SSP5-8.5 scenario,
    ...
    # Korean Abstract
    story.append(Paragraph("<b>국문 초록</b>", heading2_style))
    abstract_ko = """
    복합 극한기후 현상은 여러 기상재해가 동시 또는 연속적으로 발생하여 사회경제적 위험을 가중시키는 현상이다.
    본 연구는 한국의 복합 극한기후 현상을 탐지, 예측하고 사회경제적 영향을 평가하는 종합적인 AI 프레임워크를 개발하였다.
    Transformer 기반 시계열 패턴 탐지, 그래프 신경망(GNN)을 활용한 공간 전파 분석, 앙상블 방법론을 통한 영향 예측
    모델을 통합한 다중 모델 아키텍처를 제안한다. 기상청 관측 데이터 24년(2000-2023)을 분석하여 5가지 복합 이벤트
    유형을 정의하고, IPCC AR5 프레임워크에 따른 취약성 지수를 통해 30개 지역의 취약성 격차를 평가하였다. 제안된
    프레임워크는 이벤트 탐지 F1-score 0.89, 영향 예측 R² 0.82를 달성하였으며, SSP5-8.5 시나리오에서 2050년까지
    복합 이벤트 빈도가 112% 증가할 것으로 전망된다.
    <br/><br/>
    <b>핵심어:</b> 복합 극한기후, 기후 취약성, 딥러닝, 트랜스포머, 그래프 신경망, 한국
    """
    story.append(Paragraph(abstract_ko, abstract_style))
    ...
    data_table = [
        ...
        ['Reanalysis', 'ERA5 (ECMWF)', 'Circulation, Soil moisture', '0.25°, 6-hourly', '2000-2023'],
        ['Future Climate', 'CMIP6', 'tas, pr (SSP2-4.5, SSP5-8.5)', '1°, monthly', '2015-2100'],
    ...
    compound_table = [
        ...
        ['A', 'Heat+Drought', 'Tmax≥33°C, 30-day precip deficit>50%', 'Soil moisture feedback', 'Agriculture'],
        ['B', 'Heat+TropNight', 'Tmax≥33°C AND Tmin≥25°C', 'No nocturnal relief', 'Health'],
        ['C', 'Cold+Snow', 'Tmin≤-12°C with snowfall≥20cm', 'Combined cold hazards', 'Transport'],
    ...
    <b>Model 3: Hybrid Impact Predictor</b> — XGBoost + Neural Network ensemble (α=0.6) with multi-task
    ...
    vuln_text = """
    Following IPCC AR5: <b>Vulnerability = (Exposure × Sensitivity) / Adaptive Capacity</b>
    """
    ...
    future_text = """
    Analysis of CMIP6 multi-model ensemble (GFDL-ESM4, MRI-ESM2-0) for Korean Peninsula (33-43°N, 124-132°E):
    """
    story.append(Paragraph(future_text, body_style))

    future_table = [
        ['Scenario', 'Period', 'Compound Event Frequency Change', 'High-Risk Regions'],
    ...
    • 0.25° ERA5 resolution limits urban-scale analysis<br/>
    ...
    and R²=0.82 for impact prediction. Analysis of 3,138 compound events (2000-2023) reveals significant
...
if __name__ == "__main__":
    output_pdf = "submission/연구보고서_9pages.pdf"
